...
class CategoriasDataTable extends React.Component {
  ...
  fetchCategorias(page = 1) {
    let url =
      ...
      `/categories?page=${page}&limit=${this.state.sizePerPage}`;
    if (this.state.filter !== "") {
      url += `&filter=` + this.state.filter;
    }
    url +=
      "&sortOrder=" + this.state.sortOrder + "&sortName=" + this.state.sortName;
    axios
      ...
      .then(res => {
        if (res.data.status === "success") {
          ...
          if (res.data.data.length === 0) this.setState({
  ...
  render() {
    ...
    const lastUpdate = Moment(this.state.lastUpdate).format('DD/MM/YYYY HH:MM');
    return (
      <div className="animated fadeIn">
        <Card>
          <CardHeader>
            <h6 className={"labelAtualiza"}> Atualizado em {lastUpdate === 'Invalid date' ? '- / - / -': lastUpdate}</h6>
            ...
            >
              <thead>
                <tr>
                  ...
                  <th
                    className="tbcol-50 text-center order-column"
                    onClick={() => this.sortBy("path")}
                  >
                    Descrição
                    <span className="order">
                      <span className="dropdown">
                  ...
                  <th
                    className="tbcol-20 text-center order-column"
                    onClick={() => this.sortBy("weight")}
                  >
                    Peso
                    <span className="order">
                      <span className="dropdown">
              ...
              <tbody>
                {!isLoading ? (this.state.data === 'nenhuma categoria localizada' ? (
                <tr key={1}>
                  <td className="text-center">-</td>
                  <td className="text-center">Nenhuma categoria localizada</td>
                  <td className="text-center">-</td>
                  <td className="text-center">-</td>
                </tr>
                ):(
                  this.state.data.map(cat => {
                    return (
                      <tr key={cat.id}>
                        ...
                        <td className="text-center">{cat.path}</td>
                        <td className="text-center">{cat.weight}</td>
                        <td className="text-center">{cat.cubage}</td>
                      </tr>
                    );
                  })
                )):(
